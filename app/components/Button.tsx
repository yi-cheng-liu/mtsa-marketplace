...
interface ButtonProps{
  ...
  outline?: boolean; // is it an outline button
  small?: boolean;   // is it a small button
...
const Button: React.FC<ButtonProps> = ({ label, onClick, disabled, outline, small, icon:Icon }) => {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative disabled:opacity-50 disabled:cursor-not-allowed text-lg w-full hover:opacity-50 rounded-2xl items-center
